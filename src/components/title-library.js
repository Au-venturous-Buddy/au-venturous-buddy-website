  ...
  return(
    <>
      ...
      <Modal size="xl" show={show} onHide={handleClose} centered scrollable fullscreen={true}>
        ...
        <Modal.Body>
          <div className={`p-3 m-3 ${props.titleType}-content-main-contents`}>
            <section>
              <div className="my-3">
                <ResponsiveHeader level={2}>{props.contentName}</ResponsiveHeader>
              </div>
              <img
                className={`hover-shadow-card d-block w-100 mb-3 ${props.titleType}-content-main-contents-image`}
                src={props.contentCover}
                alt={props.contentName}
              />
  ...
  Object.keys(props.contents).forEach((group) => {
    ...
    Object.keys(contentsGroup).forEach((item) => {
      ...
      contentsRender[group][item] = (
        <ContentPreviewButton titleType={props.type} titleCode={props.titleCode} contentCover={contentsItem["cover"]} contentName={contentsItem["name"]} contentSynopsis={contentsItem["synopsis"]} contentURL={contentsItem["url"]} />
      )
    })
  })
  ...
  return(
    ...
    <SEO title={props.title} description={props.description} />
      <section className="py-3 justify-content-center" style={{textAlign: "center"}}>
        ...
        {
          Object.keys(contentsRender).map((value) => (
