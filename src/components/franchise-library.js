...
function TitlePreviewButton(props) {
  return(
    <Button aria-label={props.titleName} className={`m-2 view img-button ${props.titleType}-preview`} onClick={props.titleSlug}>
...
export default function FranchiseLibrary(props) {
  ...
  Object.keys(props.franchiseTitles).forEach((value) => {
    ...
    franchiseTitlesRender[franchiseTitle["type"]].push(
      <div>
        <TitlePreviewButton titleName={franchiseTitle["name"]} titleType={franchiseTitle["type"]} titleSlug={franchiseTitle["slug"]} titleCover={franchiseTitle["cover"]} />
      </div>
    )
  })
